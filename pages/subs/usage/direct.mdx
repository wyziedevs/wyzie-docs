...
<Important>
  An API key is required for all requests. Get a free key at [store.wyzie.io/redeem](https://store.wyzie.io/redeem) and include it as `&key=YOUR_KEY` in every request. See the [API Keys](api-keys) page for details.
</Important>
...
| **key**              | `/search?id=tt3659388&key=YOUR_KEY`                   | Your API key _(required)_. Get one free at [store.wyzie.io/redeem](https://store.wyzie.io/redeem).           |
...
| **ai**                | `true` if the entry is an AI-translated subtitle (see [Translate](translate)), `false` for normal scraped subtitles. Use it as a client-side filter when you only want one or the other. |
