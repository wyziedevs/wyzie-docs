...
The Wyzie Subs [NPM package](https://www.npmjs.com/package/wyzie-lib) provides a simple and easy-to-use interface for searching and fetching subtitles.
...
<Important>
  An API key is required for all requests. Get a free key at [store.wyzie.io/redeem](https://store.wyzie.io/redeem) and pass it via the `key` parameter. See the [API Keys](api-keys) page for details.
</Important>
...
| **key**         | key               | Your API key _(required)_. Get one free at [store.wyzie.io/redeem](https://store.wyzie.io/redeem).  |
...
Our types are very simple and well-documented. Check out the types.ts file [here](https://github.com/wyziedevs/wyzie-lib/blob/main/src/types.ts).
