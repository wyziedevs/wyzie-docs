...
<Steps>
  ...
  <Steps.Step>
    ...
    ```go
    const (
      SharedSecret = ""                         // Fallback only: set I6_SHARED_SECRET instead
  ...
  <Steps.Step>
    <T id="i6shark.hosting.stepSecret" />
    ```bash
    sudo install -m 600 /dev/null /etc/i6shark.env
    echo "I6_SHARED_SECRET=$(openssl rand -hex 32)" | sudo tee /etc/i6shark.env >/dev/null
    ```
  </Steps.Step>
  <Steps.Step>
