const messages: Record<string, string> = {
  ...
  'subs.intro.note.status':
    'Аптайм та інциденти: [sub.wyzie.io/status](https://sub.wyzie.io/status) і [API статусу](/subs/usage/status). Новини про API та магазин: [sub.wyzie.io/news](https://sub.wyzie.io/news).',
  ...
  'subs.direct.download.p':
    'Кожен url у відповіді /search вказує на https://sub.wyzie.io/c/... і містить параметр запиту tok. Параметр tok зашифрований і не розкриває ваш API-ключ; він залишається дійсним 60 днів. Кожне посилання має власний tok, який відкриває лише цей файл, тому використовуйте URL як є (додавати параметри завантаження можна). Пошук коштує 1 запит, а кожне завантаження коштує ще 1 запит, який списується з ключа, що виконав пошук. Якщо цей ключ не може оплатити завантаження, посилання відхиляється:',
  'subs.direct.dl.p':
  ...
  'subs.synced.param.media':
    'Або сам аудіо- чи відеофайл: як сире тіло запиту (інші поля тоді передаються в рядку запиту) або як multipart-поле media. До 95 MB, тому для повнометражного фільму надсилайте лише звукову доріжку. Для multipart-завантаження понад 8 MB передавайте key у рядку запиту: він перевіряється до читання файлу.',
  ...
  'subs.synced.error.401':
    'Немає ключа, посилання для завантаження в url некоректне чи прострочене, або multipart-завантаження понад 8 MB не має key у рядку запиту.',
  ...
  'subs.synced.error.429':
    'Ключ не може оплатити синхронізацію: для неї має залишитися щонайменше 5 запитів; це перевіряється до початку будь-якої роботи. Або 429 Too many syncs: ключ може запускати 60 синхронізацій на годину.',
  'subs.synced.error.503':
    'Сервер зайнятий декодуванням або читанням інших надісланих файлів або пошук тимчасово недоступний. Повторіть спробу трохи згодом або передайте speech.',
  'subs.synced.lib.p':
  ...
  'subs.status.title': 'API статусу',
  'subs.status.p1':
    'GET https://sub.wyzie.io/status/api повертає той самий статус, що й [сторінка статусу](https://sub.wyzie.io/status), у вигляді JSON для вашого власного моніторингу: чи працює API, стан кожного джерела, аптайм за 24 години, 7, 30 і 90 днів, історію по днях і нещодавні інциденти. Ключ не потрібен, і це нічого не коштує.',
  'subs.status.note':
    'Ендпоінт публічний (CORS відкритий) і кешується на 60 секунд, тому опитування частіше ніж раз на хвилину повертає ту саму відповідь. Джерела називаються кодовими назвами, як у /sources.',
  'subs.status.param.days':
    'Кількість днів щоденної історії в кожному масиві history, від найновіших: від 0 до 90 (за замовчуванням 90). Із 0 history не включається, і відповідь стає меншою.',
  'subs.status.param.format':
    'shields повертає [endpoint-бейдж shields.io](https://shields.io/badges/endpoint-badge) замість звіту.',
  'subs.status.param.source':
    'З format=shields: бейдж для одного джерела (його аптайм за 30 днів або paused) замість загального статусу.',
  'subs.status.field.status':
    'operational (усі джерела проходять перевірки), degraded (джерело призупинене або не пройшло перевірку) або partial_outage (призупинено понад половину джерел).',
  'subs.status.field.summary':
    'те саме одним реченням, наприклад "API operational; 2 of 7 sources paused".',
  'subs.status.field.trackingSince':
    'коли почалося відстеження аптайму. Раніший час не враховується, тому вікна, що сягають далі в минуле, охоплюють менше (або дорівнюють null).',
  'subs.status.field.api':
    'сам API: його uptime і history. В отриманій вами відповіді status завжди дорівнює operational.',
  'subs.status.field.sources':
    'по одному запису на джерело: tier (free або paid), статус останньої перевірки для фільмів і серіалів, latencyMs, lastChecked і nextCheck, а також uptime і history.',
  'subs.status.field.state':
    'online, suspect (не пройшло одну перевірку; повторна перевірка протягом 5 хвилин) або paused (не пройшло дві перевірки поспіль). Призупинене джерело має listed: false: воно виключається з /sources і source=all, доки не пройде перевірку, а pausedSince показує, відколи.',
  'subs.status.field.uptime':
    'відсоток кожного вікна, протягом якого API або джерело працювали, округлений униз до 3 знаків після коми (тож будь-який простій дає значення нижче 100), або null, якщо даних ще немає.',
  'subs.status.field.history':
    'по одному запису на день UTC: date, uptime і downMinutes (null до початку відстеження).',
  'subs.status.field.incidents':
    'призупинення джерел за останні 30 днів, від найновіших: start, end (null, поки триває) і minutes.',
  'subs.status.how.api':
    'Аптайм API: поки API працює, сервер щохвилини записує heartbeat. Хвилина без нього вважається простоєм. Вимірювання відбувається на нашому сервері, тому проблема, що існує лише між вами та Cloudflare, тут не відобразиться.',
  'subs.status.how.sources':
    'Аптайм джерел: кожне джерело перевіряється щогодини справжнім пошуком і завантаженням. Час, поки джерело призупинене, вважається простоєм, починаючи з першої невдалої перевірки й до успішної. Одна невдала перевірка сама по собі не враховується, як і джерело, яке ми призупинили вручну.',
  'subs.status.how.tracking': 'Відстеження почалося 24 вересня 2026 року.',
  'subs.status.badge.p':
    'Додайте ?format=shields, щоб отримати бейдж shields.io для вашого README або сторінки статусу:',
  'subs.status.use.p':
    'Для вибору джерел у вашому застосунку /sources уже перелічує лише активні. API статусу потрібен, щоб показувати користувачам, що працює, сповіщати себе або вирішувати, коли повторити спробу:',
  'subs.status.news.p':
    'Оголошення про API та магазин (нові функції, зміни, що стосуються вашого застосунку) публікуються на [sub.wyzie.io/news](https://sub.wyzie.io/news), електронною поштою, якщо ви там підпишетеся, або через [RSS](https://sub.wyzie.io/news/feed.xml).',
  ...
  'i6shark.intro.feature2':
    '**HTTP-методи**: GET, HEAD і POST; усе інше отримує 405',
  ...
  'i6shark.intro.feature5':
    "**Безпечна обробка запитів**: пересилаються лише заголовки запиту з дозволеного списку і ніколи не пересилаються токен API чи заголовки Cloudflare та проксіювання (forwarding). Адреси призначення в loopback-, приватних, link-local та інших внутрішніх мережах (включно з власними адресами сервера) відхиляються після DNS-резолвінгу та за кожного редиректу (не більше 5). Підтримка кількох форматів параметрів URL. Необов'язковий відкат на системний IP за замовчуванням.",
  ...
  'i6shark.hosting.step2.note':
    'Оновіть IPv6Prefix та Interface відповідно до вашого сервера. Спільний секрет задається в оточенні (наступний крок), а не в цьому файлі; SharedSecret є лише запасним варіантом, якщо I6_SHARED_SECRET не задано. Решта констант налаштування мають розумні значення за замовчуванням і зазвичай не потребують змін.',
  'i6shark.hosting.stepSecret':
    'Помістіть спільний секрет у файл оточення, доступний лише root. Використовуйте те саме значення у своєму клієнті (для Wyzie Subs це I6_PROXY_SECRET):',
  ...
  'i6shark.hosting.auth.p':
    'API-токени генеруються за допомогою HMAC-SHA256 на основі спільного секрету (I6_SHARED_SECRET) за заголовком user-agent і надсилаються в заголовку API-Token, який проксі ніколи не пересилає далі. Деталі реалізації — у функції validateAPIToken у вихідному коді. Якщо секрет колись витече, задайте новий і перезапустіть сервіс.',
