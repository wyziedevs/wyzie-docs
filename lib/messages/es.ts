const messages: Record<string, string> = {
  ...
  // Index / Landing Page
  'index.subtitle':
    'Wyzie es un conjunto de herramientas para buscar subtítulos y todo lo relacionado.',
  'index.cta.start': 'Comenzar',
  'index.cta.store': 'Échale un Vistazo',

  'index.card.keys.title': 'Claves API Gratuitas',
  'index.card.keys.body':
    'Obtén una clave API gratuita en store.wyzie.io/redeem con una rápida verificación por correo electrónico (Gmail, Outlook, Yahoo, iCloud, Proton y otros proveedores importantes). 1.000 solicitudes/día sin costo. Planes de pago disponibles para mayor uso.',

  'index.card.ai.title': 'Traducción con IA',
  'index.card.ai.body':
    'Traduce cualquier subtítulo a más de 80 idiomas bajo demanda. El SRT se transmite en orden a medida que terminan los lotes, así que las primeras líneas llegan rápido. Disponible con claves Pro.',
  ...
  'subs.intro.p1':
    'Wyzie Subs es una API de scraping de subtítulos con un nivel gratuito. Hay dos formas de hacer solicitudes a la API: usando nuestro paquete NPM o consultando directamente la API de Wyzie. Recomiendo usar nuestro paquete, aunque algunos pueden encontrar los tipos engorrosos. Para usar la API, primero debes tomar esa decisión.',
  'subs.intro.important.apikey':
    'Se requiere una clave API para todas las solicitudes. Obtén una clave gratuita en [store.wyzie.io/redeem](https://store.wyzie.io/redeem) (verificación por correo electrónico, 1.000 solicitudes/día). Para mayor uso, hay [planes Pro y de recarga](https://store.wyzie.io) disponibles. Consulta la página de Claves API para más detalles.',
  'subs.intro.note.npm':
    'Recomendamos encarecidamente el paquete NPM si estás familiarizado con TypeScript o JavaScript',
  'subs.intro.note.status':
    'Tiempo de actividad e incidentes: [sub.wyzie.io/status](https://sub.wyzie.io/status) y la [API de estado](/subs/usage/status). Novedades sobre la API y la tienda: [sub.wyzie.io/news](https://sub.wyzie.io/news).',
  ...
  'subs.intro.protect.opt1.p1':
    'Wyzie Worker es un proxy ligero de Cloudflare Worker que inyecta tu clave API en el servidor. Despliégalo en Cloudflare Workers y configura dos secretos: `NITRO_API_TOKEN` (tu clave API de Wyzie) y `NITRO_WORKER_KEY` (un secreto que tú eliges). Cada solicitud al worker debe enviar `Authorization: Bearer <NITRO_WORKER_KEY>`; sin ella, el worker responde 401, y si `NITRO_WORKER_KEY` no está configurado, rechaza todas las solicitudes con 503. La clave del worker también debe quedarse en el servidor, así que llama al worker desde tu propio backend, nunca desde código del navegador o de una aplicación. El worker reenvía cada solicitud a sub.wyzie.io con tu clave API adjunta.',
  ...
  'subs.sources.ai.desc':
    'No es un scraper. SRT generado bajo demanda con IA y traducido a partir del mejor subtítulo fuente disponible. Solo para claves Pro. Consulta la guía de Traducción con IA para todos los detalles.',
  'subs.sources.ai.content':
    'Cualquier contenido para el que Wyzie pueda encontrar un subtítulo de texto',
  ...
  'subs.pkg.param.hi':
    'Si es true, devuelve solo subtítulos para personas con discapacidad auditiva. Las fuentes que no marcan los subtítulos para personas con discapacidad auditiva no devuelven nada.',
  'subs.pkg.param.source':
    'Proveedores de subtítulos a consultar por nombre en clave (all para todas las fuentes activas que tu clave puede usar; por defecto charlie).',
  ...
  'subs.pkg.helpers':
    'El paquete también incluye helpers ligeros de TMDB: searchTmdb, getTvDetails y getSeasonDetails para encontrar IDs rápidamente antes de llamar a /search. getSources devuelve los nombres en clave de las fuentes activas (una fuente pausada por sus comprobaciones de estado se omite hasta que se recupera), y getSourcesInfo devuelve la respuesta completa de /sources con los niveles y, si le pasas una clave, qué fuentes puede usar esa clave. withDownloadOptions añade opciones de descarga (salida WebVTT, correcciones de tiempos, un segundo idioma y más) a la url de un resultado.',
  ...
  'subs.pkg.type.download':
    'Opciones para withDownloadOptions: to, offset, fps, plain y (Pro) sdh, clean, dual.',
  'subs.pkg.type.sync':
    'Entrada y resultado de syncSubtitle (Wyzie Synced, claves Pro): qué subtítulo (un resultado, su url, o tmdb_id/imdb_id con language), los segmentos de voz (speech) que encontró detectSpeech o el archivo multimedia (media), y el enlace de descarga sincronizado con su offset, fps y confidence. Consulta [Wyzie Synced](/subs/usage/synced).',
  'subs.pkg.types.end':
    'Nuestros tipos son muy simples y están bien documentados. Consulta [src/types.ts](https://github.com/wyziedevs/wyzie-lib/blob/main/src/types.ts) en el repositorio de wyzie-lib.',
  ...
  'subs.direct.param.hi':
    'Si es true, devuelve solo subtítulos para personas con discapacidad auditiva (es un filtro, no una preferencia). Las fuentes que no marcan los subtítulos para personas con discapacidad auditiva no devuelven nada.',
  'subs.direct.param.encoding': 'Filtro de codificación de caracteres.',
  'subs.direct.param.source':
    'Proveedores de subtítulos a consultar (all consulta todas las fuentes que tu clave puede usar; por defecto charlie).',
  ...
  'subs.direct.param.page':
    'Página a devolver, empezando por 1. Solo se usa junto con limit.',
  'subs.direct.param.limit':
    'Resultados por página (de 1 a 200). Sin él, todos los resultados se devuelven en una sola respuesta.',
  ...
  'subs.direct.data.url':
    'Enlace de descarga en https://sub.wyzie.io/c/... que lleva un parámetro tok cifrado. Cada descarga cuesta 1 solicitud; consulta la sección sobre la descarga de subtítulos más abajo.',
  ...
  'subs.direct.download.p':
    'Cada url de una respuesta de /search apunta a https://sub.wyzie.io/c/... y lleva un parámetro de consulta tok. tok está cifrado, así que no revela tu clave API, y sigue siendo válido durante 60 días. Cada enlace tiene su propio tok, que solo abre ese archivo, así que usa la URL tal cual (agregar opciones de descarga no es problema). Una búsqueda cuesta 1 solicitud y cada descarga, 1 más; se cobran a la clave que hizo la búsqueda. Cuando esa clave no puede pagar una descarga, el enlace se rechaza:',
  'subs.direct.dl.p':
    'Agrégalos a una URL de descarga para cambiar lo que devuelve. Funcionan en todas las descargas, estén en caché o no, y no cuestan nada extra (salvo dual, más abajo). La cabecera de respuesta X-Subtitle-Transforms indica lo que se aplicó, con recuentos.',
  'subs.direct.dl.param.to':
    'Formato de salida: `srt` o `vtt`. `vtt` se reproduce directamente en un elemento `<track>` del navegador. Por defecto: el formato propio del archivo.',
  'subs.direct.dl.param.offset':
    'Desplaza todas las líneas esta cantidad de segundos (un valor negativo las adelanta).',
  'subs.direct.dl.param.fps':
    'Corrige el desfase progresivo de un subtítulo hecho para otro lanzamiento: `SUBTITLE_FPS:VIDEO_FPS`, p. ej. `25:23.976` para un subtítulo PAL en un video a velocidad de cine.',
  'subs.direct.dl.param.plain':
    'Líneas simples y limpias: se eliminan los códigos de estilo como `{\\an8}` y `<font>`, se descartan las líneas vacías y repetidas, las líneas se ordenan por tiempo y se recortan los pequeños solapamientos.',
  'subs.direct.dl.param.sdh':
    'Elimina el texto para personas con discapacidad auditiva: `[DOOR SLAMS]`, `(sighs)`, etiquetas de hablante como `JOHN:` y letras de canciones con ♪.',
  'subs.direct.dl.param.clean':
    'Censura las groserías fuertes, conservando la primera letra (`f***`). Solo archivos en inglés.',
  'subs.direct.dl.param.dual':
    'Añade un segundo idioma (código ISO 639-1) debajo de cada línea, alineado con los tiempos de este archivo. Cuesta 1 solicitud extra, solo cuando se encuentra una coincidencia; si no, el archivo se devuelve sin el segundo idioma y con `X-Dual: unavailable`.',
  'subs.direct.dl.after':
    'Las opciones se pueden combinar, p. ej. `&to=vtt&sdh=strip&offset=-1.5`. Los enlaces ya incluyen `format`, `encoding`, `id` y (para episodios) `season` y `episode`: déjalos como están. `autoUnzip=false` devuelve un archivo comprimido tal cual.',
  'subs.direct.oneCall.p':
    'Con una clave API, GET /download devuelve directamente el archivo de subtítulos en una sola llamada: busca con tu clave y los mismos parámetros que /search (language es en por defecto), elige la mejor coincidencia y la sirve. Cuesta 2 solicitudes, lo mismo que una búsqueda más una descarga. Las opciones de descarga como to y offset se aplican al archivo.',
  'subs.direct.oneCall.pick':
    'La mejor coincidencia es el primer resultado de búsqueda, con preferencia por los archivos SRT, WebVTT y ASS salvo que indiques format, y por los archivos sin texto para personas con discapacidad auditiva salvo que indiques hi=true. Acota la elección con release, filename, source u origin. Las cabeceras de respuesta X-Subtitle-Release, X-Subtitle-Source, X-Subtitle-Language y X-Subtitle-Url indican qué archivo se eligió. Los errores son los mismos que para /search y los enlaces de descarga.',
  'subs.direct.oneCall.keyless':
    'Sin clave, la [página de descarga](https://sub.wyzie.io/download) sirve para encontrar uno o dos subtítulos a mano. Sus enlaces solo abren el archivo para el que se crearon, desde la red que hizo la búsqueda, y la página tiene límites por hora. Para cualquier cosa automatizada, usa una clave.',
  'subs.direct.headers.p':
    'Cada respuesta de /search incluye una cabecera X-Total-Count con el número total de resultados. Cuando pasas limit, también incluye:',
  'subs.direct.header.xpage': 'la página devuelta.',
  'subs.direct.header.xperpage': 'el valor de limit aplicado.',
  'subs.direct.header.xtotalpages': 'el número total de páginas.',
  'subs.direct.headers.rate':
    'Las respuestas también incluyen X-RateLimit-Limit, X-RateLimit-Remaining y X-RateLimit-Reset. Tómalas como valores aproximados: el uso se liquida con la facturación en pequeños lotes, por lo que pueden ir ligeramente por detrás de tu uso real.',

  // Subs Translate Page
  'subs.translate.title': 'Traducción de Subtítulos con IA',
  'subs.translate.important':
    'La traducción con IA es una **función Pro**; las claves gratuitas reciben 403 Upgrade required. Cada llamada cuesta **25 solicitudes** del saldo de tu clave, incluidos los aciertos de caché. Si una llamada falla antes de producir cualquier salida (no se encuentra ningún subtítulo, falla una búsqueda o una descarga, o el servidor está ocupado), las 25 solicitudes se reembolsan automáticamente.',
  'subs.translate.p1':
    'Wyzie puede traducir cualquier subtítulo a más de 80 idiomas al instante. El SRT traducido se transmite en orden a medida que terminan los lotes, así que las primeras entradas llegan rápido en lugar de hacerlo cuando ya está listo todo el archivo. La traducción completa se almacena en caché durante 30 días, por lo que las solicitudes posteriores para el mismo título, episodio e idioma de destino se sirven desde la caché.',
  ...
  'subs.translate.way1.p1':
    'Para las claves Pro, cada respuesta de /search también incluye filas de traducción con IA con "ai": true y una url que apunta a /translate: una por cada idioma compatible, o solo los idiomas de tu filtro language=. Las claves gratuitas nunca reciben estas filas. Trata las filas de IA como cualquier otra fila de subtítulos en tu interfaz: cuando el usuario haga clic en una, obtén la URL.',
  ...
  'subs.translate.param.target':
    'Idioma de destino (requerido): un nombre de la lista de idiomas compatibles (p. ej. Spanish, Japanese, Portuguese (Brazil)) o su código (p. ej. es, ja, pt-BR).',
  ...
  'subs.translate.param.tk':
    'Token cifrado de las URL de las filas de IA en /search. Funciona como key, no revela tu clave API y sigue siendo válido durante 60 días.',
  ...
  'subs.translate.how.step1':
    'Wyzie busca en las fuentes normales un subtítulo de texto, prefiriendo un SRT en inglés cuando está disponible. Los archivos VTT, ASS, SSA y SUB se convierten primero a SRT.',
  'subs.translate.how.step2':
    'El SRT se divide en lotes de hasta unos 3.800 caracteres y se traduce con Google Translate, 4 lotes a la vez.',
  'subs.translate.how.step3':
    'La salida se transmite en el orden del SRT a medida que terminan los lotes, así que las primeras entradas llegan rápido. Los reproductores que aceptan un cuerpo SRT en streaming pueden comenzar a mostrar las primeras líneas antes de que el resto esté listo.',
  ...
  'subs.translate.languages.p':
    'Más de 80 idiomas, incluyendo todos los principales idiomas europeos, asiáticos, africanos y de Oriente Medio. Pasa un nombre de la lista (Spanish, Portuguese (Brazil)) o su código (es, pt-BR). Para las claves Pro, la lista completa también se devuelve como filas ai: true en cualquier respuesta de /search sin filtro language=, que es la fuente de verdad canónica.',

  'subs.translate.limitations.h2': 'Limitaciones',
  'subs.translate.limit1':
    'La traducción con IA necesita un subtítulo de texto como punto de partida. Las fuentes VTT, ASS, SSA y SUB se convierten primero a SRT; si no existe ningún subtítulo de texto, la llamada devuelve 404 No subtitle found y las 25 solicitudes se reembolsan.',
  ...
  'subs.translate.limit4':
    'Las traducciones se cobran también en aciertos de caché. Ya sea generada de nuevo o servida desde la caché de 30 días, cada llamada a /translate cuesta 25 solicitudes. Solo se reembolsan las llamadas que fallan antes de producir cualquier salida.',

  // Subs Synced Page
  'subs.synced.title': 'Wyzie Synced',
  'subs.synced.important':
    'Wyzie Synced es una **función Pro**: las claves gratuitas reciben 403 Paid feature. Cada sincronización correcta cuesta **5 solicitudes**; una sincronización que no encuentra coincidencia no se cobra. Después, descargar el enlace sincronizado cuenta como cualquier otra descarga.',
  'subs.synced.p1':
    'Los subtítulos que encuentras en internet suelen estar sincronizados para un lanzamiento distinto del video que tienes: empiezan unos segundos antes o después, o se van desfasando a medida que avanza la película porque ese lanzamiento usa otra velocidad de fotogramas. Wyzie Synced escucha el audio de tu copia, detecta dónde habla la gente y calcula el desplazamiento (offset) y la corrección de velocidad de fotogramas que alinean el subtítulo con él. Obtienes un enlace de descarga normal con la corrección aplicada (las [opciones de descarga](/subs/usage/direct#download-options) offset y fps).',
  'subs.synced.web.p':
    'La forma más sencilla: abre [sub.wyzie.io/synced](https://sub.wyzie.io/synced), ingresa tu clave Pro, elige tu archivo de video y el título, y descarga el subtítulo sincronizado. El audio se analiza en tu navegador, así que el video nunca se sube: solo se envían los tiempos del habla. Funcionan MKV, MP4, AVI y la mayoría de los demás formatos, incluido el audio AC3, E-AC3 y DTS.',
  'subs.synced.api.p':
    'Envía qué subtítulo quieres (un enlace de descarga, o el título para que Wyzie elija la mejor coincidencia) y el audio: ya sea los tiempos de habla que detectaste tú mismo, o el propio archivo de audio/video. POST /synced es la misma API.',
  'subs.synced.param.url':
    'Un enlace de descarga de /search (https://sub.wyzie.io/c/…). Las demás opciones de descarga que lleve (to, sdh, …) se conservan en el enlace sincronizado.',
  'subs.synced.param.id':
    'En lugar de url: ID de TMDB o IMDB. Wyzie prueba los 5 mejores subtítulos de texto en ese idioma y devuelve el que mejor encaja con tu audio.',
  'subs.synced.param.language':
    'Con id: código ISO 639-1 del idioma del subtítulo (requerido).',
  'subs.synced.param.seasonEpisode':
    'Con id, para TV. Ambos deben estar presentes juntos.',
  'subs.synced.param.key':
    'Tu clave API Pro. Sin ella, se usa la clave vinculada al tok de url; los enlaces de la página de descarga sin clave necesitan key.',
  'subs.synced.param.speech':
    'Dónde habla la gente: [[start, end], …] en segundos, de cualquier detector de actividad de voz (detectSpeech de wyzie-lib, Silero VAD, webrtcvad). Una película de 2 horas genera unos 2.000 segmentos, alrededor de 40 KB de JSON.',
  'subs.synced.param.media':
    'O el propio archivo de audio/video: como cuerpo en bruto de la solicitud (con los demás campos en la cadena de consulta), o como el campo multipart media. Hasta 95 MB, así que para una película completa sube solo la pista de audio. En una subida multipart de más de 8 MB, pon key en la cadena de consulta: se comprueba antes de leer el archivo.',
  'subs.synced.fields.note':
    'Los campos van en un cuerpo JSON, un formulario multipart o la cadena de consulta (con media enviado en bruto como cuerpo).',
  'subs.synced.response.p': 'Una respuesta 200 es JSON:',
  'subs.synced.field.url':
    'el enlace de descarga del subtítulo con la corrección de tiempos (offset, fps) y un tok nuevo para tu clave. Úsalo como cualquier url de /search: cada descarga cuesta 1 solicitud.',
  'subs.synced.field.offset':
    'segundos que se suman a cada línea después de la corrección de velocidad de fotogramas (un valor negativo las adelanta).',
  'subs.synced.field.fps':
    'la corrección de velocidad de fotogramas como SUBTITLE_FPS:VIDEO_FPS (p. ej. "25:23.976"), o null cuando no hizo falta ninguna.',
  'subs.synced.field.confidence':
    'de 0 a 1: con qué claridad este ajuste de tiempos supera a cualquier otro. Todo lo que se devuelve ya ha pasado la prueba de coincidencia; cuanto más alto, más seguro.',
  'subs.synced.field.inSync':
    'true cuando el subtítulo ya coincidía con tu copia.',
  'subs.synced.field.subtitle':
    'qué subtítulo se usó (release, fileName, format, source, …). Con url, solo incluye format.',
  'subs.synced.errors.p':
    'Los errores son JSON con message y details. Las sincronizaciones rechazadas o fallidas no se cobran.',
  'subs.synced.error.400':
    'Campos ausentes o no válidos: sin subtítulo, sin audio, o un speech que no está formado por pares [start, end].',
  'subs.synced.error.401':
    'Falta la clave, el enlace de descarga de url no es válido o ha caducado, o una subida multipart de más de 8 MB no lleva key en la cadena de consulta.',
  'subs.synced.error.403':
    'La clave es gratuita (Wyzie Synced requiere Pro), no es válida o está en pausa.',
  'subs.synced.error.404':
    'No hay subtítulos de texto en ese idioma para el título.',
  'subs.synced.error.413':
    'El archivo media supera los 95 MB. Sube solo la pista de audio o envía speech.',
  'subs.synced.error.422':
    'El subtítulo no se alinea con el audio con ningún desplazamiento ni velocidad de fotogramas (probablemente es de otro montaje u otro episodio), el audio tiene muy poca voz, o el archivo no se puede decodificar.',
  'subs.synced.error.429':
    'La clave no puede pagar: una sincronización necesita que queden al menos 5 solicitudes, y se comprueba antes de empezar cualquier trabajo. O 429 Too many syncs: una clave puede iniciar 60 sincronizaciones por hora.',
  'subs.synced.error.503':
    'El servidor está ocupado decodificando o leyendo otros archivos subidos, o la búsqueda no está disponible momentáneamente. Reintenta en breve o envía speech.',
  'subs.synced.lib.p':
    'wyzie-lib incluye detectSpeech (el mismo detector que el sitio ejecuta en tu navegador) y syncSubtitle:',
  'subs.synced.how.step1':
    'Voz: el audio se decodifica a 8 kHz mono (solo el canal central en las mezclas 5.1 y 7.1, que es donde está el diálogo), y un detector de actividad de voz marca dónde habla la gente: sonido intenso en la banda de la voz que sube y baja con las sílabas.',
  'subs.synced.how.step2':
    'Alineación: los tiempos en pantalla del subtítulo se correlacionan de forma cruzada con esa voz en cada desplazamiento dentro de ±10 minutos, para las diferencias habituales de velocidad de fotogramas (25 frente a 23.976, 25 frente a 24, 24 frente a 23.976 fps).',
  'subs.synced.how.step3':
    'Refinamiento: el mejor ajuste se afina hasta 10 ms alineando el inicio de las líneas con el inicio de la voz.',
  'subs.synced.how.step4':
    "Solo se devuelve un ajuste cuando destaca claramente sobre cualquier otro desplazamiento, así que un subtítulo de otro montaje u otro episodio recibe 422 Couldn't sync en lugar de un desplazamiento incorrecto.",
  'subs.synced.limit1':
    'Wyzie Synced corrige un desfase constante y una diferencia de velocidad de fotogramas. Un subtítulo de otro montaje (con escenas añadidas o eliminadas) no se puede corregir con un único desplazamiento, y se rechaza.',
  'subs.synced.limit2':
    'Necesita voz: las películas con poco diálogo, o con un audio que es principalmente música, puede que no se sincronicen.',
  'subs.synced.limit3': 'Detecta desfases de hasta ±10 minutos.',

  // Subs Status API Page
  'subs.status.title': 'API de estado',
  'subs.status.p1':
    'GET https://sub.wyzie.io/status/api devuelve el mismo estado que muestra la [página de estado](https://sub.wyzie.io/status), en JSON para tu propia monitorización: si la API está activa, el estado de cada fuente, el tiempo de actividad en 24 horas, 7, 30 y 90 días, un historial día a día y los incidentes recientes. No necesita clave y no cuesta nada.',
  'subs.status.note':
    'Es público (CORS abierto) y se almacena en caché durante 60 segundos, así que consultarlo más de una vez por minuto devuelve la misma respuesta. Las fuentes se identifican por su nombre en clave, como en /sources.',
  'subs.status.param.days':
    'Días de historial diario en cada array history, del más reciente al más antiguo: de 0 a 90 (por defecto 90). Con 0 se omite history para obtener una respuesta más pequeña.',
  'subs.status.param.format':
    'shields devuelve una [insignia de endpoint de shields.io](https://shields.io/badges/endpoint-badge) en lugar del informe.',
  'subs.status.param.source':
    'Con format=shields: una insignia para una sola fuente (su tiempo de actividad a 30 días, o paused) en lugar del estado general.',
  'subs.status.field.status':
    'operational (todas las fuentes pasan las comprobaciones), degraded (una fuente pausada o que falla una comprobación) o partial_outage (más de la mitad de las fuentes pausadas).',
  'subs.status.field.summary':
    'lo mismo en una frase, p. ej. "API operational; 2 of 7 sources paused".',
  'subs.status.field.trackingSince':
    'cuándo empezó el seguimiento del tiempo de actividad. El tiempo anterior no se cuenta, así que las ventanas que llegan más atrás cubren menos (o son null).',
  'subs.status.field.api':
    'la propia API: su uptime e history. status siempre es operational en una respuesta que hayas recibido.',
  'subs.status.field.sources':
    'una entrada por fuente: tier (free o paid), el estado de la última comprobación para películas y TV, latencyMs, lastChecked y nextCheck, además de uptime e history.',
  'subs.status.field.state':
    'online, suspect (falló una comprobación; se vuelve a comprobar en menos de 5 minutos) o paused (falló dos comprobaciones seguidas). Una fuente pausada tiene listed: false: queda fuera de /sources y de source=all hasta que pase una comprobación, y pausedSince indica desde cuándo.',
  'subs.status.field.uptime':
    'porcentaje de cada ventana en que la API o la fuente estuvo activa, redondeado hacia abajo a 3 decimales (así cualquier caída aparece por debajo de 100), o null si aún no hay datos.',
  'subs.status.field.history':
    'una entrada por día UTC: date, uptime y downMinutes (null antes de que empezara el seguimiento).',
  'subs.status.field.incidents':
    'pausas de fuentes de los últimos 30 días, de la más reciente a la más antigua: start, end (null mientras continúa) y minutes.',
  'subs.status.how.api':
    'Tiempo de actividad de la API: mientras la API funciona, el servidor registra un latido (heartbeat) cada minuto. Un minuto sin latido cuenta como caída. Se mide en nuestro servidor, así que un problema que solo exista entre tú y Cloudflare no aparecerá aquí.',
  'subs.status.how.sources':
    'Tiempo de actividad de las fuentes: cada fuente se comprueba cada hora con una búsqueda y una descarga reales. El tiempo que una fuente está pausada cuenta como caída, desde su primera comprobación fallida hasta que pasa una comprobación. Una sola comprobación fallida no cuenta, ni tampoco una fuente que pausamos a mano.',
  'subs.status.how.tracking':
    'El seguimiento empezó el 24 de septiembre de 2026.',
  'subs.status.badge.p':
    'Agrega ?format=shields para obtener una insignia de shields.io para tu README o tu página de estado:',
  'subs.status.use.p':
    'Para elegir fuentes en tu app, /sources ya lista solo las activas. La API de estado sirve para mostrar a tus usuarios qué está funcionando, avisarte a ti mismo o decidir cuándo reintentar:',
  'subs.status.news.p':
    'Los anuncios sobre la API y la tienda (nuevas funciones, cambios que afectan a tu app) se publican en [sub.wyzie.io/news](https://sub.wyzie.io/news), por correo electrónico si te suscribes allí, o por [RSS](https://sub.wyzie.io/news/feed.xml).',
  ...
  'subs.keys.tier.free': 'Gratuito (verificación por correo electrónico)',
  ...
  'subs.keys.free.step2':
    'Ingresa una dirección de un proveedor importante de correo personal (Gmail, Outlook/Hotmail, Yahoo, iCloud, AOL, Proton y otros). Se rechazan los dominios de correo desechable.',
  ...
  'subs.keys.free.gmail':
    'Cada dirección de correo electrónico y cada red pueden obtener una clave gratuita; una segunda solicitud devuelve 409. ¿Perdiste tu clave? Usa "Forgot key" en el [panel de control](https://store.wyzie.io/dashboard) para que te la reenvíen.',
  ...
  'subs.keys.protect.option1':
    'Usa [Wyzie Worker](https://github.com/wyziedevs/wyzie-worker): un proxy gratuito de Cloudflare Worker que guarda tu clave API como el secreto `NITRO_API_TOKEN`. Cada llamada a él debe enviar `Authorization: Bearer <NITRO_WORKER_KEY>`, un segundo secreto que tú configuras, así que haz pasar tu cliente por tu propio backend y mantén también la clave del worker en el servidor.',
  ...
  'subs.keys.protect.devtools':
    'Si la clave aparece en una pestaña de red en DevTools, está expuesta. Asume que es pública y rótala desde tu [panel de control](https://store.wyzie.io/dashboard).',
  ...
  'subs.keys.limit.p':
    'Una búsqueda cuesta 1 solicitud y cada descarga de subtítulo cuesta 1 solicitud, así que buscar una vez y descargar un archivo consume 2. La traducción con IA cuesta 25 solicitudes por llamada, y una sincronización de Wyzie Synced, 5.',
  'subs.keys.limit.free':
    '**Nivel gratuito** agotado -> las búsquedas y los enlaces de descarga devuelven 429 Daily request limit reached, con reset_at en el JSON y una cabecera Retry-After. El límite diario de 1.000 solicitudes se restablece a medianoche UTC.',
  'subs.keys.limit.paid':
    '**Saldo de pago** agotado -> las búsquedas y los enlaces de descarga devuelven 402 con un enlace de recarga en el JSON. Recarga en [store.wyzie.io/topup](https://store.wyzie.io/topup) o habilita la **recarga automática** en tu panel de control para rellenar automáticamente cuando tu saldo cruce un umbral que tú configures.',
  'subs.keys.hold.p1':
    'Las claves que envían un volumen muy alto, sobre todo desde IPs de centros de datos o de hosting, se pausan automáticamente. Una clave pausada recibe 403 Key on hold en cada solicitud, con un enlace para reactivarla (https://store.wyzie.io/verify) y un enlace de soporte (https://store.wyzie.io/contact) en el JSON.',
  'subs.keys.hold.p2':
    'Para reactivar la clave de inmediato, verifica el sitio web donde la usas en [store.wyzie.io/verify](https://store.wyzie.io/verify) con un registro DNS TXT o una etiqueta meta. Una clave con un sitio verificado nunca vuelve a pausarse automáticamente, así que los sitios con mucho tráfico pueden verificarse antes de que se pausen.',
  'subs.keys.hold.p3':
    '¿No tienes un sitio web (por ejemplo, un servicio de backend o una app)? [Contacta al soporte](https://store.wyzie.io/contact) para que reactiven la clave.',

  'subs.keys.files.h2': 'Qué Hay en los Archivos',
  'subs.keys.files.adfilter':
    '**Filtrado de anuncios** – a todos los subtítulos servidos a través de sub.wyzie.io se les quitan las entradas publicitarias de los proveedores (banners de OpenSubtitles, promociones de apuestas, líneas del tipo "mira gratis en ..."). Las entradas SRT se renumeran para que la numeración no tenga saltos. Todos los proveedores, incluido OpenSubtitles, se sirven a través de sub.wyzie.io, así que el filtro se aplica a todos ellos.',
  'subs.keys.files.promo':
    '**Las claves gratuitas y de desarrollo** reciben una breve entrada al principio de cada archivo (0–6 s) que apunta a [store.wyzie.io](https://store.wyzie.io). Las claves de pago reciben archivos limpios, sin esa entrada.',

  'subs.keys.faq.h2': 'Preguntas Frecuentes',
  'subs.keys.faq.q1': 'Perdí mi clave. ¿Puedo obtener una nueva?',
  'subs.keys.faq.a1':
    'Abre el [panel de control](https://store.wyzie.io/dashboard) y usa "Forgot key" con tu correo registrado; te reenviaremos tu clave existente. Si crees que la clave se filtró, rótala desde el panel de control en su lugar.',
  ...
  'i6shark.intro.feature2':
    '**Métodos HTTP**: GET, HEAD y POST; cualquier otro recibe un 405',
  ...
  'i6shark.intro.feature5':
    '**Manejo Seguro de Solicitudes**: Solo se reenvían las cabeceras de solicitud de una lista permitida, nunca el token de API ni las cabeceras de Cloudflare y de reenvío. Se rechazan los destinos en redes loopback, privadas, link-local y otras redes internas (incluidas las propias direcciones del servidor), después de la resolución DNS y en cada redirección (como máximo 5). Soporte para múltiples formatos de parámetros URL. Respaldo opcional a la IP predeterminada del sistema.',
  ...
  'i6shark.intro.req1': 'Go 1.22 o superior',
  'i6shark.intro.req2':
  ...
  'i6shark.hosting.step2.note':
    'Actualiza IPv6Prefix e Interface para que coincidan con tu servidor. El secreto compartido va en el entorno (siguiente paso), no en este archivo; SharedSecret solo es un respaldo cuando I6_SHARED_SECRET no está definido. Las demás constantes de ajuste tienen valores predeterminados razonables y generalmente no necesitan cambios.',
  'i6shark.hosting.stepSecret':
    'Guarda el secreto compartido en un archivo de entorno que solo pueda leer root. Usa el mismo valor en tu cliente (para Wyzie Subs, I6_PROXY_SECRET):',
  ...
  'i6shark.hosting.auth.p':
    'Los tokens de API se generan usando HMAC-SHA256 con el secreto compartido (I6_SHARED_SECRET) sobre la cabecera user-agent, y se envían en la cabecera API-Token, que el proxy nunca reenvía al destino. Consulta la función validateAPIToken en el código fuente para los detalles de implementación. Si alguna vez se filtra un secreto, define uno nuevo y reinicia el servicio.',
  ...
  'plugins.index.shared.sources':
    '**Fuentes:** todas las fuentes que tu clave puede usar (`source=all`): charlie y lima con una clave gratuita, las siete con una clave Pro.',
  ...
  'plugins.index.outro':
    'Elige tu plataforma arriba para empezar. El código fuente de todos los plugins está en el [repositorio de wyzie-plugins](https://github.com/wyziedevs/wyzie-plugins).',

  'plugins.stremio.intro':
    'Un complemento de subtítulos de un solo clic para [Stremio](https://www.stremio.com/). Consulta todas las fuentes de Wyzie que tu clave puede usar y funciona tanto para películas como para series, en todas las plataformas en las que se ejecuta Stremio.',
  ...
  'plugins.stremio.install.4':
    'Opcional: activa la opción **discapacidad auditiva** para obtener solo subtítulos para personas con discapacidad auditiva. Déjala desactivada para verlo todo; mientras esté activada, las fuentes que no marcan los subtítulos para personas con discapacidad auditiva no devuelven nada.',
  ...
  'plugins.stremio.cfg.hi.d':
    'Devolver solo subtítulos para personas con discapacidad auditiva (envía hi=true). Desactivado por defecto.',
  ...
  'plugins.bazarr.intro':
    '[Bazarr](https://www.bazarr.media/) gestiona los subtítulos de **Plex, Jellyfin, Emby, Sonarr y Radarr** en un solo lugar. Añadir Wyzie como proveedor da a todos esos servidores acceso a todas las fuentes de Wyzie que tu clave puede usar, a través de una única clave.',
  'plugins.bazarr.note':
  ...
  'plugins.bazarr.install.1':
    'Descarga [bazarr/wyzie.py](https://github.com/wyziedevs/wyzie-plugins/blob/main/bazarr/wyzie.py) del [repositorio de wyzie-plugins](https://github.com/wyziedevs/wyzie-plugins) (o `git clone https://github.com/wyziedevs/wyzie-plugins.git`) y cópialo a `bazarr/subliminal_patch/providers/wyzie.py`.',
  ...
  'plugins.bazarr.cfg.hi':
    'Devolver solo subtítulos para personas con discapacidad auditiva (envía hi=true).',
  ...
  'plugins.bazarr.quota.401':
    '**401** (falta la clave) o **403** (clave desconocida o en pausa): Bazarr muestra un error de autenticación para que sepas que debes revisar la clave o volver a introducirla.',
  ...
  'plugins.kodi.zip.1':
    'Consigue el zip del complemento: `service.subtitles.wyzie-<version>.zip`. Para compilarlo tú mismo, clona el [repositorio de wyzie-plugins](https://github.com/wyziedevs/wyzie-plugins) (`git clone https://github.com/wyziedevs/wyzie-plugins.git`) y comprime su carpeta `kodi/` para que el zip contenga `addon.xml` en su raíz.',
  'plugins.kodi.zip.2':
