const messages: Record<string, string> = {
  ...
  'subs.intro.note.status':
    'وقت التشغيل والأعطال: [sub.wyzie.io/status](https://sub.wyzie.io/status) و[API الحالة](/subs/usage/status). أخبار الـ API والمتجر: [sub.wyzie.io/news](https://sub.wyzie.io/news).',
  ...
  'subs.synced.param.media':
    'أو ملف الصوت/الفيديو نفسه: كجسم طلب خام (مع بقية الحقول في سلسلة الاستعلام)، أو كحقل multipart باسم media. الحد الأقصى 95 MB، لذا في حالة الفيلم الكامل ارفع المسار الصوتي وحده. عند الرفع بصيغة multipart بحجم يتجاوز 8 MB، ضع key في سلسلة الاستعلام: إذ يُتحقَّق منه قبل قراءة الملف.',
  ...
  'subs.synced.error.401':
    'لا يوجد مفتاح، أو رابط التنزيل في url غير صالح أو منتهي الصلاحية، أو أن رفعاً بصيغة multipart يتجاوز 8 MB لا يحتوي على key في سلسلة الاستعلام.',
  ...
  'subs.synced.error.429':
    'لا يستطيع المفتاح الدفع: تحتاج المزامنة إلى 5 طلبات متبقية على الأقل، ويُتحقَّق من ذلك قبل بدء أي عمل. أو 429 Too many syncs: يمكن للمفتاح بدء 60 مزامنة في الساعة.',
  'subs.synced.error.503':
    'الخادم مشغول بفك ترميز ملفات مرفوعة أخرى أو قراءتها، أو البحث غير متاح مؤقتاً. أعد المحاولة بعد قليل، أو أرسل speech.',
  'subs.synced.lib.p':
  ...
  'subs.status.title': 'API الحالة',
  'subs.status.p1':
    'يعرض GET https://sub.wyzie.io/status/api الحالة نفسها التي تعرضها [صفحة الحالة](https://sub.wyzie.io/status)، بصيغة JSON لمراقبتك الخاصة: هل الـ API يعمل، وحالة كل مصدر، ووقت التشغيل خلال 24 ساعة و7 و30 و90 يوماً، وسجل يومي، والأعطال الأخيرة. لا حاجة إلى مفتاح، ولا يكلف شيئاً.',
  'subs.status.note':
    'نقطة النهاية عامة (CORS مفتوح) ومخزّنة مؤقتاً لمدة 60 ثانية، لذا فإن الاستعلام أكثر من مرة في الدقيقة يعيد الإجابة نفسها. تُسمّى المصادر بأسمائها الرمزية، كما في /sources.',
  'subs.status.param.days':
    'عدد أيام السجل اليومي في كل مصفوفة history، الأحدث أولاً: من 0 إلى 90 (الافتراضي 90). القيمة 0 تحذف history للحصول على استجابة أصغر.',
  'subs.status.param.format':
    'تُعيد القيمة shields [شارة endpoint من shields.io](https://shields.io/badges/endpoint-badge) بدلاً من التقرير.',
  'subs.status.param.source':
    'مع format=shields: شارة لمصدر واحد (وقت تشغيله خلال 30 يوماً، أو paused) بدلاً من الحالة العامة.',
  'subs.status.field.status':
    'operational (كل المصادر تجتاز الفحوصات)، أو degraded (مصدر متوقف مؤقتاً أو يفشل في فحص)، أو partial_outage (أكثر من نصف المصادر متوقفة مؤقتاً).',
  'subs.status.field.summary':
    'المعلومة نفسها في جملة واحدة، مثل "API operational; 2 of 7 sources paused".',
  'subs.status.field.trackingSince':
    'متى بدأ تتبع وقت التشغيل. لا يُحتسب الوقت السابق، لذا فالنوافذ الزمنية التي تمتد أبعد إلى الوراء تغطي مدة أقل (أو تكون null).',
  'subs.status.field.api':
    'الـ API نفسه: قيم uptime وhistory الخاصة به. تكون قيمة status دائماً operational في أي استجابة تصلك.',
  'subs.status.field.sources':
    'إدخال واحد لكل مصدر: tier (free أو paid)، وحالة آخر فحص للأفلام والمسلسلات التلفزيونية، وlatencyMs، وlastChecked وnextCheck، إضافة إلى uptime وhistory.',
  'subs.status.field.state':
    'online، أو suspect (فشل في فحص واحد؛ يُعاد فحصه خلال 5 دقائق)، أو paused (فشل في فحصين متتاليين). المصدر المتوقف مؤقتاً تكون قيمته listed: false: فيُستبعد من /sources ومن source=all حتى ينجح أحد الفحوصات، ويوضح pausedSince منذ متى.',
  'subs.status.field.uptime':
    'النسبة المئوية من كل نافذة زمنية التي كان فيها الـ API أو المصدر يعمل، مقرّبة للأسفل إلى 3 منازل عشرية (بحيث يظهر أي توقف أقل من 100)، أو null إذا لم تتوفر بيانات بعد.',
  'subs.status.field.history':
    'إدخال واحد لكل يوم بتوقيت UTC: date وuptime وdownMinutes (قيمته null قبل بدء التتبع).',
  'subs.status.field.incidents':
    'فترات توقف المصادر خلال آخر 30 يوماً، الأحدث أولاً: start وend (قيمته null ما دام التوقف مستمراً) وminutes.',
  'subs.status.how.api':
    'وقت تشغيل الـ API: ما دام الـ API يعمل، يسجّل الخادم نبضة (heartbeat) كل دقيقة. تُحتسب الدقيقة التي تخلو من نبضة وقتَ توقف. يُقاس ذلك على خادمنا، لذا فإن أي مشكلة تقع بينك وبين Cloudflare فقط لن تظهر هنا.',
  'subs.status.how.sources':
    'وقت تشغيل المصادر: يُفحص كل مصدر كل ساعة ببحث وتنزيل حقيقيين. يُحتسب الوقت الذي يكون فيه المصدر متوقفاً مؤقتاً وقتَ توقف، بدءاً من أول فحص فاشل حتى ينجح أحد الفحوصات. لا يُحتسب فحص فاشل واحد بمفرده، ولا المصدر الذي نوقفه يدوياً.',
  'subs.status.how.tracking': 'بدأ التتبع في 24 سبتمبر 2026.',
  'subs.status.badge.p':
    'أضف ?format=shields للحصول على شارة shields.io لملف README أو صفحة الحالة الخاصة بك:',
  'subs.status.use.p':
    'لاختيار المصادر في تطبيقك، تعرض /sources بالفعل المصادر النشطة فقط. أما API الحالة فهو لعرض ما يعمل لمستخدميك، أو لتنبيه نفسك، أو لتحديد متى تعيد المحاولة:',
  'subs.status.news.p':
    'تُنشر الإعلانات المتعلقة بالـ API والمتجر (الميزات الجديدة، والتغييرات التي تؤثر في تطبيقك) على [sub.wyzie.io/news](https://sub.wyzie.io/news)، وعبر البريد الإلكتروني إذا اشتركت هناك، أو عبر [RSS](https://sub.wyzie.io/news/feed.xml).',
  ...
  'i6shark.intro.feature2':
    '**أساليب HTTP**: GET وHEAD وPOST؛ وأي أسلوب آخر يتلقى 405',
  ...
  'i6shark.intro.feature5':
    '**معالجة آمنة للطلبات**: لا تُمرَّر إلا ترويسات الطلب الموجودة في قائمة مسموح بها، ولا يُمرَّر أبداً رمز API ولا ترويسات Cloudflare وترويسات التمرير (forwarding). تُرفض الوجهات على شبكات loopback والشبكات الخاصة وlink-local وغيرها من الشبكات الداخلية (بما في ذلك عناوين الخادم نفسه)، بعد تحليل DNS وعند كل إعادة توجيه (5 كحد أقصى). دعم صيغ متعددة لمعاملات URL. احتياطي اختياري إلى IP النظام الافتراضي.',
  ...
  'i6shark.hosting.step2.note':
    'حدّث IPv6Prefix وInterface لتتناسب مع خادمك. يُوضع السر المشترك في البيئة (الخطوة التالية) لا في هذا الملف؛ وSharedSecret مجرد قيمة احتياطية عندما لا يكون I6_SHARED_SECRET مضبوطاً. بقية ثوابت الضبط الدقيق لها قيم افتراضية معقولة ولا تحتاج عادةً إلى تغيير.',
  'i6shark.hosting.stepSecret':
    'ضع السر المشترك في ملف بيئة لا يقرؤه إلا root. استخدم القيمة نفسها في عميلك (بالنسبة إلى Wyzie Subs: I6_PROXY_SECRET):',
  ...
  'i6shark.hosting.auth.p':
    'تُولَّد رموز API باستخدام HMAC-SHA256 مع السر المشترك (I6_SHARED_SECRET) على ترويسة وكيل المستخدم، وتُرسَل في ترويسة API-Token، التي لا يمررها الوسيط أبداً إلى الخادم الوجهة. راجع دالة validateAPIToken في الكود المصدري لتفاصيل التنفيذ. إذا تسرّب السر يوماً، فاضبط سراً جديداً وأعد تشغيل الخدمة.',
